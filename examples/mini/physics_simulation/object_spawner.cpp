...
ObjectSpawner::ObjectSpawner(LTEngine::Physics::PhysicsEngine *physics) : m_physicsEngine(physics) {
	...
	m_spawnTimer.onEnd += [this]() {
		LTEngine::Rendering::ColorA randomColor = LTEngine::Rendering::ColorA(rand() % 256, rand() % 256, rand() % 256, 255);
		i32 spawnPositionX = rand() % (i32)(m_spawnAreaWidth + 1 - position.x) + (i32)position.x;

		const u32 object =
		    getObjectStructure()->addObject(std::make_unique<Square>(m_physicsEngine, randomColor, spawnPositionX, position.y));
		m_objects.push_back(object);
		m_objectTimers[object].onEnd += [this, object]() {
...
ObjectSpawner::~ObjectSpawner() {
	m_spawnTimer.stop();
	for (u32 object : m_objects) { getObjectStructure()->removeObject(object); }
	m_objects.clear();
