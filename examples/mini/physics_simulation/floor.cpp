...
void Floor::render(LTEngine::Rendering::Renderer *renderer) {
	...
	renderer->drawRect({0, 0, (f32)m_size.x, (f32)m_size.y}, LTEngine::Rendering::ColorA(30, 30, 30, 255));
}


std::unique_ptr<LTEngine::Object::ObjectStructure::Object> Floor::clone() const {
	return std::make_unique<Floor>(m_physicsEngine, (LTEngine::Shapes::Rect){position.x, position.y, (f32)m_size.x, (f32)m_size.y});
}
