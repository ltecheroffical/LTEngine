...
PlatformRandomSecure::PlatformRandomSecure(RandomSettings settings) : _random(settings.unix_random_buffer_size) {
}
#else
...
u8 PlatformRandomSecure::next_u8() {
	return _random.next_u8();
}

u16 PlatformRandomSecure::next_u16() {
	return _random.next_u16();
}

u32 PlatformRandomSecure::next_u32() {
	return _random.next_u32();
}

u64 PlatformRandomSecure::next_u64() {
	return _random.next_u64();
}


i8 PlatformRandomSecure::next_i8() {
	return _random.next_i8();
}

i16 PlatformRandomSecure::next_i16() {
	return _random.next_i16();
}

i32 PlatformRandomSecure::next_i32() {
	return _random.next_i32();
}

i64 PlatformRandomSecure::next_i64() {
	return _random.next_i64();
}


f32 PlatformRandomSecure::next_f32() {
	return _random.next_f32();
}

f64 PlatformRandomSecure::next_f64() {
	return _random.next_f64();
}


void PlatformRandomSecure::next_bytes(void *data, size_t size) {
	_random.next_bytes(data, size);
}
