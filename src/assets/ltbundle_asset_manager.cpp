...
const std::vector<u8> LTBundleAssetManager::loadPure(const std::string &path) {
	...
	if (!file.is_open()) { throw std::runtime_error("Could not open bundle file!"); }
	...
	if (header.version != LTBUNDLE_VERSION) { throw std::runtime_error("Invalid LTBundle version!"); }

	for (u32 i = 0; i < header.assetCount; i++) {
		LTBundleAssetEntry entry;
		file.read((char *)&entry, sizeof(LTBundleAssetEntry));

		if (entry.pathSize + entry.size > fileSize) { throw std::runtime_error("Bundle file is corrupt or not a LTBundle file!"); }
		...
		if (entry_path == path) {
			...
			if (entry.checksum != Hash::crc32(data.data(), entry.size)) { throw std::runtime_error("Asset is corrupt!"); }
			return data;
		} else {
