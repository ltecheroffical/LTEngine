...
const std::vector<u8> AssetManager::loadAsset(std::string path) {
	...
	if (m_cache.contains(path)) {
		return std::move(m_cache[path].data);
	}
	...
	try {
		...
		if (m_postProcessor == nullptr || !m_postProcessor->assetLoadFailed(path, &data)) {
			throw;
		}
	}
	if (m_postProcessor != nullptr) {
		m_postProcessor->postProcess(path, &data);
	}
...
void AssetManager::saveAsset(std::string path, const u8 *data, size_t size) {
	...
	if (m_postProcessor != nullptr) {
		m_postProcessor->preProcess(path, &processedData);
	}
	...
	if (m_controller(path, data, size)) {
		if (m_cache.contains(path)) {
			...
			m_cache[path] = CachedAsset{
			    ...
			    .data = processedData,
			    .lastAccessed = static_cast<u64>(std::chrono::steady_clock::now().time_since_epoch().count())};
			m_usedCacheMemory += processedData.size();
		}
	}
...
void AssetManager::cacheEvictor() {
	...
	for (it = m_cache.begin(); it != m_cache.end(); it++) {
		if (m_evictor(&it->second, m_usedCacheMemory)) {
			m_cache.erase(it);
			it = m_cache.begin();
		}
	}
