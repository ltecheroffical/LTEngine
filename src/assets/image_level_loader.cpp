...
void ImageLevelLoader::buildLevel(std::function<void(u32 x, u32 y, u32 tile)> addObjCallback) {
	if (m_image == nullptr) { throw std::runtime_error("No level loaded!"); }
	for (u32 y = 0; y < m_image->getSize().x; y++) {
		for (u32 x = 0; x < m_image->getSize().y; x++) {
			if (getTile(x, y) == TILE_NONE) { continue; }
...
u32 ImageLevelLoader::getTile(u32 x, u32 y) {
	if (m_image == nullptr) { throw std::runtime_error("No level loaded!"); }
	if (m_image->getPixel(x, y).a == 0) { return TILE_NONE; }
	if (!m_tileMap.contains(m_image->getPixel(x, y))) { return TILE_NONE; }
